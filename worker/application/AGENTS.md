...
- `cache/` — `invalidateTestCaches`, `refreshPublishedTestsIndexSnapshot`.
...
- Cache invalidation on public detail/list change: delete KV `test:<id>` and `tests:index`, refresh R2 `assets/index.json`, and purge Cache API entries for `/api/tests` and `/api/tests/:id`.
