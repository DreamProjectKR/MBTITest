...
import { JSON_HEADERS, getImagesPrefix, readTest, writeTest } from "../../../../utils/store.js";
...
export async function onRequestPut(
  ...
  const mbtiRaw = context.params?.mbti ? String(context.params.mbti).trim().toUpperCase() : "";
  if (!mbtiRaw || !MBTI_ORDER.includes(mbtiRaw as (typeof MBTI_ORDER)[number])) {
  ...
  try {
    await bucket.put(key, bytes, { httpMetadata: { contentType: upload.contentType } });
  } catch (err) {
    return json(
  ...
  const results = t.results && typeof t.results === "object" ? (t.results as Record<string, unknown>) : {};
  const existing = results[mbtiRaw] && typeof results[mbtiRaw] === "object" ? (results[mbtiRaw] as Record<string, unknown>) : {};
  results[mbtiRaw] = { ...existing, image: `${getImagesPrefix(testId)}${fileName}` };
  t.results = results;
