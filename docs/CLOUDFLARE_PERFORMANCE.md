...
## 9. Cloudflare MCP 기준선 (2026-03-07)

Cloudflare MCP(`plugin-cloudflare-cloudflare-bindings`, `plugin-cloudflare-cloudflare-observability`)로 현재 기준선을 수집했습니다.

- Worker: `mbtitest-api`
- D1:
  - `tests_count`: 2
  - `test_images` rows: 60
- R2 bucket: `mbti-assets` (region: APAC)
- KV namespace: `MBTI_KV`
- Observability:
  - 최근 7일 `mbtitest-api` 집계 조회에서 request count 0으로 확인됨
  - 의미: 현재는 Worker Observability 로그가 거의 없거나, 수집 설정/트래픽 볼륨이 낮아 p90/p99 비교가 어려운 상태

## 10. 응답 헤더 계측 표준

성능 비교를 위해 다음 헤더를 표준으로 사용합니다.

- `X-MBTI-Edge-Cache`
  - `HIT`: Cache API hit
  - `MISS`: Cache miss 후 오리진(D1/R2) 처리
  - `BYPASS`: KV hit 또는 캐시 우회 경로
- `Server-Timing`
  - API/에셋에서 `cache`, `kv`, `d1`, `r2`, `total` 항목을 상황별로 노출
  - 예: `cache;dur=0.8;desc="HIT", total;dur=1.2`

## 11. MCP 관측 쿼리 템플릿

### 11.1 키 확인

- 도구: `observability_keys`
- 필터: `$metadata.service = mbtitest-api`
- timeframe: 최근 1시간 또는 배포 전/후 동일 범위

### 11.2 지연 집계

- 도구: `query_worker_observability`
- view: `calculations`
- calculations:
  - `count($metadata.latency)`
  - `avg($metadata.latency)`
  - `p90($metadata.latency)`
  - `p99($metadata.latency)`

### 11.3 엔드포인트/트리거 분포

- 도구: `observability_values`
...
1. 캐시/프리로드 변경 배포
2. 배포 직후 10~30분 MCP 재측정(동일 timeframe 폭 유지)
3. 다음 항목이 개선되면 유지:
   - Start -> quiz 전환 체감 지연
   - 첫 질문 이미지 표시 시간
   - `X-MBTI-Edge-Cache`에서 HIT 비율 증가
4. 이상 없으면 전체 트래픽 유지
...
- 이미지 누락/404 증가
- quiz 진입 지연 급증
- 캐시 일관성 문제(관리자 저장 후 반영 지연)

롤백 우선순위:

1. SW 전략(`stale-while-revalidate`) 이전 버전으로 복원
2. 캐시 조회 순서 변경 이전 커밋으로 복원
3. 에셋 키 정규화 변경 이전 커밋으로 복원
