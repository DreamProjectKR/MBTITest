...
published 상태인 테스트 상세 정보를 반환합니다. D1 메타 + R2 본문을 병합합니다.
...
**캐싱**: KV (TTL 300s) → Cache API (ETag) → D1+R2
...
- `X-MBTI-Edge-Cache`
  - `BYPASS`: KV 히트
  - `HIT`: Cache API 히트
  - `MISS`: KV/Cache 미스 후 D1+R2 조회
- `Server-Timing`: `kv`, `cache`, `d1`, `r2`, `total`

**ETag 지원**: `If-None-Match` 헤더로 `304 Not Modified` 응답

**공개 정책**:

- `isPublished=false`인 테스트는 `404 Not Found`를 반환합니다.
- draft 편집 조회는 `GET /api/admin/tests`, `GET /api/admin/tests/:id`를 사용합니다.
...
**공개 정책**: `isPublished=false`인 테스트는 `404 Not Found` (`GET /api/tests/:id`와 동일).

**제한** (상수는 [worker/api/\_utils/rateLimit.ts](../worker/api/_utils/rateLimit.ts), [worker/api/\_utils/bodyLimits.ts](../worker/api/_utils/bodyLimits.ts)에서 조정):

- IP당 속도: 분당 10회 (`MBTI_KV` 고정 윈도, 초과 시 `429 Too Many Requests`)
- 요청 본문 최대 32KiB (`413 Payload Too Large`)
...
**헤더**: `Cache-Tag`, `ETag`, `Cache-Control`, `Vary: Accept-Encoding`, `X-MBTI-Edge-Cache`, `Server-Timing`
...
draft와 published를 모두 포함한 테스트 목록을 반환합니다. 어드민 편집기 초기 로딩에 사용합니다.
...
**캐싱**: 없음 (`Cache-Control: no-store`)

---

### `GET /api/admin/tests/:id`

draft와 published를 모두 포함한 테스트 상세 정보를 반환합니다. 어드민 편집기 상세 로딩에 사용합니다.
...
- IP당 속도: 분당 30회 (`429`)
- 단일 파일 최대 6MiB (`413`)
- `image/svg+xml` 업로드 불가 (`415 Unsupported Media Type`)
...
- 중간 실패 시 업로드된 이미지와 `test.json`은 가능한 범위에서 롤백합니다.
